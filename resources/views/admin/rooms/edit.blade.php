...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2>Edit Kamar</h2>
        <p class="text-muted mb-0">Ubah spesifikasi dan status kamar #{{ $room->id }}</p>
    </div>
            ...
            <div class="card-body p-4">
                <form action="{{ route('admin.rooms.update', $room) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control rounded-3 @error('room_number') is-invalid @enderror" id="room_number" name="room_number" value="{{ old('room_number', $room->room_number) }}" required placeholder="Contoh: 101, A-02">
                        @error('room_number')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control rounded-3 @error('floor') is-invalid @enderror" id="floor" name="floor" value="{{ old('floor', $room->floor) }}" min="1" placeholder="Contoh: 1">
                        @error('floor')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select rounded-3 @error('status') is-invalid @enderror" id="status" name="status" required>
                            ...
                            <option value="occupied" {{ old('status', $room->status) === 'occupied' ? 'selected' : '' }}>Occupied (Terisi)</option>
                            <option value="booked" {{ old('status', $room->status) === 'booked' ? 'selected' : '' }}>Booked (Terpesan)</option>
                            <option value="maintenance" {{ old('status', $room->status) === 'maintenance' ? 'selected' : '' }}>Maintenance (Perawatan)</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
