        ...
        <div class="se-order-card">
            <div class="se-order-header">
                <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
                    <div>
                        <h5 class="fw-bold mb-1" style="color: var(--text);">#{{ $order->order_number }}</h5>
            ...
            <div class="se-order-body">
                @foreach($order->details->take(3) as $detail)
                    <div class="se-order-item">
                        ...
                        <div class="flex-grow-1">
                            <div style="font-weight: 600; font-size: 0.95rem; color: var(--text);">{{ $detail->menu->name }}</div>
                            <small class="text-muted" style="font-family:var(--font-alt);">Qty: x{{ $detail->quantity }}</small>
                        </div>
                ...
                @if($order->details->count() > 3)
                    <small class="text-muted ms-5" style="font-family:var(--font-alt);">+{{ $order->details->count() - 3 }} item lainnya</small>
                @endif
            </div>
            ...
            <div class="se-order-footer">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <span class="text-muted small" style="font-family:var(--font-alt);">
                            <i class="bi bi-{{ $order->order_type === 'dine_in' ? 'building' : 'box' }} me-1"></i>
                            {{ $order->order_type === 'dine_in' ? 'Dine In' : 'Takeaway' }}
                        </span>
                        @if($order->table_number)
                            <span class="text-muted small ms-3" style="font-family:var(--font-alt);">
                                <i class="bi bi-door-open me-1"></i>{{ $order->table_number }}
                            </span>
                        @endif
                    </div>
                    <div class="text-end">
                ...
                <div class="d-grid gap-2 mt-3">
                    <a href="{{ route('customer.restaurant.order.detail', $order) }}" class="btn-se btn-se-primary" style="padding:10px 18px;font-size:0.85rem;">
    ...
    @empty
        <div class="se-card-lg text-center py-5">
            <div class="py-4">
                ...
                <a href="{{ route('customer.restaurant.menu') }}" class="btn-se btn-se-primary">
