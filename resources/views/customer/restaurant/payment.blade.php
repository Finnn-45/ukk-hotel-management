...
@section('title', 'Pembayaran - Restoran StayEase')
        ...
        <div class="payment-card">
            ...
            <div class="order-info">
                ...
                <div class="order-info-row">
                    ...
                    <span class="order-info-value">
                        @if($order->order_type === 'dine_in')
                            🍴 Dine In (Kamar {{ $order->table_number }})
                ...
                <div class="order-info-row">
                    ...
                    <span class="order-info-value">{{ $order->details->sum('quantity') }} item</span>
                </div>
                <div class="order-info-row" style="border-top: 1px solid #BAE6FD; padding-top: 12px; margin-top: 12px;">
                    ...
                    <span class="order-info-value order-total">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</span>
                </div>
            </div>
            ...
            <button class="btn-back" onclick="window.location.href='{{ route('customer.restaurant.orders') }}'">
...
document.getElementById('payButton').addEventListener('click', function() {
    ...
    snap.pay('{{ $snapToken }}', {
        onSuccess: function(result) {
            window.location.href = '{{ route("customer.restaurant.payment.success") }}?order_id=' + result.order_id + '&status_code=' + result.status_code;
        },
        onPending: function(result) {
            window.location.href = '{{ route("customer.restaurant.payment.pending") }}?order_id=' + result.order_id;
        },
        onError: function(result) {
            window.location.href = '{{ route("customer.restaurant.payment.error") }}';
        },
        onClose: function() {
